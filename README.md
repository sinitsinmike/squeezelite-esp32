...
```
type=[WS2812|APA102],length=<n>,gpio=<dataPin>,[clk=<clockPin>][,seq=<seq>][,scale=<gain>]
```
where `<n>` is the number of LEDs in the strip (1..255).  The Clock pin is required for APA102 LEDs.  For APA102 LEDs, the first 10 sequence can be defined (for example, `,seq=2106543`).  A `<scale>` gain value (percentage) can be added to enhance effect responses. 
